...
final class Spell5 {
	...
	private static Vector corrections = new Vector();  // List of potential corrections  FIXME, this needs to be a Hashtable (can get multiple same words)
...
System.out.println("deleting --------------> " + curr_word[curr_max]);
					  unused_prefix.addElement(curr_word[curr_max]);
					}
				}
...
//							  diph_bucket[curr_max].remove(curr_word[curr_max]);
						...
						//curr_max++;  // I don't this is the right place for this
						try {
							while (!finished) {
								curr_pos++;
								if (curr_max < curr_pos) {
System.out.println("unused_prefixes = " + unused_prefix);
									for (int p = 0; p < unused_prefix.size(); p++) {
System.out.println("really removing! ----> " + unused_prefix.elementAt(p));
									...
									unused_prefix = new Vector(); // is this fastest way to do this
									curr_max = curr_pos;
								}
//		System.out.println("curr_pos = " + curr_pos);
								if (poss4pos[curr_pos].hasMoreElements()) {
