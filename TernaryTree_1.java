...
import java.io.*;
   ...
   public synchronized void insert(String s) {
     ...
     root = insert1(root, s.toCharArray());
   }

	protected String insertstr;

  private synchronized TernaryLeaf insert1(TernaryLeaf p, char[] s) {
    if (p == null) {
      p = new TernaryLeaf();
      p.splitchar = s[0];
    ...
    if (s[0] < p.splitchar) 
      ...
      if (s[0] == 0)
        p.s = insertstr.toCharArray();
      else {
        // create substring and pass -->
  ...
  public void pmsearch(String s) {
    pmsearch(root, new String(s + '\0').toCharArray());
  }

  private void pmsearch(TernaryLeaf p, char[] s) {
    if (p == null) return;
    // nodecnt++;  // uncomment to keep track of how many nodes touched
    if ((s[0] == '.') || (s[0] < p.splitchar))
      pmsearch(p.lokid, s);
    if ((s[0] == '.') || (s[0] == p.splitchar))
      if ((p.splitchar != 0) && (s[0] != 0)) {
        // create and pass -->
        pmsearch(p.eqkid, s.substring(1));
      }
    if ((s[0] == 0) && (p.splitchar == 0))
  ...
  public boolean sssearch(String s) {
   ...
   private boolean rsearch(TernaryLeaf p, char[] s) {
     if  (p == null) return false;
     if (s[0] < p.splitchar)
   ...
   public void nearsearch(String s, int d) {
     s = s + '\0';
     nearsearch(root, s.toCharArray(), d);
   }

   private void nearsearch(TernaryLeaf p, char[] s, int d) {
     if ((p == null) || (d < 0)) return;
     if ((d > 0) || (s[0] < p.splitchar))
     ...
     if (p.splitchar == 0) {
       ...
       nearsearch(p.eqkid, (s[0] == 0) ? s.substring(1) : s,
         (s[0] == p.splitchar) ? d : d-1);
     if ((d > 0) || (s[0] > p.splitchar))
